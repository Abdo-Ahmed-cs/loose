    ...
    return (
        <nav className='w-full flex justify-end bg-n-10 px-[15px] py-[30px] relative'>
            <div className='flex gap-2 items-center cursor-pointer' onClick={() => setOpen(prev => !prev)}>
                <Avatar src='' className='w-[40px] aspect-square' />
                <span className='text-n-100 text-sm font-semibold'>Abdullah Mohamed</span>
                <KeyboardArrowDownIcon className={`ml-5 translate-y-[2px] ${open ? "" : "rotate-180"}`} />
            </div>
            {open && (
                <div className='absolute right-[32px] bottom-[-140px] w-[270px] bg-n-10 rounded-[10px] p-2 flex flex-col gap-2'>
